...
class Sandbox():
    ...
    def __init__(self) -> None:
        ...
        self.topic_map: Dict[str, Callable[[dict], None]] = {
            ...
            f"{self.topic_prefix}/fcc/velocity": self.show_velocity,
            f"{self.topic_prefix}/fcc/velocity": self.velocitynum,
    ...
    # the console.
    def velocitynum(self, data: dict) -> None:
        self.vx = data["vX"]
        self.vy = data["vY"]
        self.vz = data["vZ"]
    ...
    def send_color_by_distance_mesg(self,distance):
            ...
            yellow = Color("green")
            colors = list(yellow.range_to(Color("purple"),35))
    ...
    def visible_apriltag(self,data: dict) -> None:
            ...
            """velocity = data[0]
            print(velocity)
            vx = velocity["velocity"]["vX"]
            vy = velocity["velocity"]["vY"]
            vz = velocity["velocity"]["vZ"]"""
            #self.show_velocity(data)
            """self.velocitynum(data)
            if (int(vx) < .127) and (int(vy) < .127):
                velocity_drop = True
            else:
                velocity_drop = False"""
            velocity_drop = True
            ...
            #Handle building/package drops  
                ...
                if tagid == 1:
                    distance = (((10)-xPos)*((10)-xPos)) + (((30)-yPos)*((30)-yPos))
                    distance = math.sqrt(distance)
                elif tagid == 2:
                    distance = (((20)-xPos)*((20)-xPos)) + (((1)-yPos)*((1)-yPos))
                    distance = math.sqrt(distance)
                elif tagid == 3:
                    distance = (((-16)-xPos)*((-16)-xPos)) + (((27)-yPos)*((27)-yPos))
                    distance = math.sqrt(distance)
                ...
                if (distance<8):
                    ...
                    self.send_color_by_distance_mesg(distance)  
